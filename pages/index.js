...
function validate(address, amount, currentAmount) {
  ...
  if (amount < 1 || (parseInt(amount, 10)) > 999) {
    stats.eAmount = true;
  }
  return stats;
}
  ...
  return (
    <Root>
      <div className={classes.header}>
        <Typography className={classes.font}>BSC Testnet USDT Faucet</Typography>
      </div>
      <div className={classes.mainCont}>
        <Paper className={classes.inputCont}>
          ...
          <TextField required id={'amount'} label={'Token Amount'} type={'number'}
            error={amountErr}
            helperText={amountErr ? 'can only mint up to 1 - 999 TUSDT' : ''}
            className={classes.input}
            fullWidth
            value={amount}
            inputProps={{min: 1}}
